...
def total_episode_reward_logger(rew_acc, rewards, masks, writer, steps, tag="rewards/episode_reward"):
    """
...
#     # logger.log("In Episode Logger ({})\n\trew_acc: {}\n\trewards: {}\n\tmasks: {}\n\tsteps: {}".format(tag, rew_acc, rewards, masks, steps))
#     with tf.variable_scope("environment_info", reuse=True):
#         for env_idx in range(rewards.shape[0]):
#             dones_idx = np.sort(np.argwhere(masks[env_idx]))
#
